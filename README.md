...
Posts:
  Meta:
    ...
    Replication:
     async master-master между ДЦ. 
     Между репликами сделаем в рамках одного ДЦ сделаем по строгому коворому sync master-slave чтобы как минимум одна W реплика ответила ОК. Для понижения latency можем дождаться ответа от реплики что она записала хотя бы в WAL журнал. Пользователь будет читать свой пост из реплики куда пишет
...
Comments:
  Meta:
    ...
    Replication:
     async master-master между ДЦ. 
     В рамках одного ДЦ реплика будет async master-slave по не строгому коворуму, потеря комментария не страшна, сделаем ниже latency. Пользователь будет читать свой комментарий из реплики куда пишет. 
  ...
  Hosts: 
    ...
    Replication:
     async master-master между ДЦ
     Между одним ДЦ async master-slave
